...
public class HttpManager {
    ...
    private OkHttpClient getOkHttpClientWithCache() {
        ...
        //设置缓存
        File httpCacheDirectory = new File(ShdyApplication.getInstance().getApplicationContext().getCacheDir(), "dmgAppcache");
        builder.cache(new Cache(httpCacheDirectory, 10 * 1024 * 1024));
        builder.addNetworkInterceptor(new CacheInterceptor(ShdyApplication.getInstance().getApplicationContext()));
        builder.addInterceptor(new NormalInterceptor(ShdyApplication.getInstance().getApplicationContext()));
